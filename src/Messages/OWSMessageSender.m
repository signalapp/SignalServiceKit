                          ...
                          failure:(void (^)(NSError *error))failureHandler
{
    ...
    dispatch_async([OWSDispatch sendingQueue], ^{
        ...
        RetryableFailureHandler markAndFailureHandler = ^(NSError *error, BOOL isRetryable) {
            [self saveMessage:message withError:error];
            if (isRetryable) {
                DDLogWarn(@"Skipping retry for message failure in this corner case.");
                // Fixing this will require a larger refactor. In the meanwhile, we don't
          ...
          failure:(RetryableFailureHandler)failureHandler
{
    ...
    if (blockedCount > 0 && futures.count == 0) {
        NSError *error = OWSErrorMakeMessageSendFailedToBlocklistError();
        // No point in retrying, since people will continue to be blocked.
        failureHandler(error, NO);
        return;
    }
